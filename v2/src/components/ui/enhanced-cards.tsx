...
interface EnhancedCardProps {
  ...
  delay?: number;
  hoverScale?: number;
  glowColor?: 'primary' | 'accent' | 'secondary' | 'destructive';
}
...
export function EnhancedCard({
  ...
  const glowColors = {
    ...
    accent: 'hover:shadow-lg hover:shadow-accent/25',
    secondary: 'hover:shadow-lg hover:shadow-secondary/25',
    destructive: 'hover:shadow-lg hover:shadow-destructive/25',
  };

  const cardContent = (
    <Card
      className={cn(
        'border-border/50 h-full backdrop-blur-sm transition-all duration-300',
  ...
  if (!isMounted) {
    return <div className={cn('group cursor-pointer', className)}>{cardContent}</div>;
  }
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={cn('group gpu-accelerated cursor-pointer will-change-transform', className)}
    >
      {cardContent}
    </motion.div>
  );
